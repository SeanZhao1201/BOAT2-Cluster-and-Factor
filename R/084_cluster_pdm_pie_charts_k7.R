# R/084_cluster_pdm_pie_charts_k7.R
# 此脚本创建饼图来可视化每个簇(k=7)中PDM经验的分布
# 数据从脚本081的输出加载

# 1. 加载Setup和数据 -----------------------------------------------------
cat("============== SCRIPT R/084 (PDM Experience Pie Charts k7) STARTING ==============\n")
...
# 为此脚本的结果创建子目录(084)
FIG_DIR_084 <- paste0("results/figures/084_pdm_experience_pie_charts_k", K_VALUE_FIXED)
if (!dir.exists(FIG_DIR_084)) {
  dir.create(FIG_DIR_084, recursive = TRUE)
  cat(paste("Created directory:", FIG_DIR_084, "\n"))
}
...
cat("Configuration complete. Output will be saved to:", FIG_DIR_084, "\n")
...
create_pdm_pie_charts <- function(pdm_var) {
  ...
  for (cluster_id in sorted_clusters) {
    ...
    # 保存单个饼图
    individual_plot_filename <- file.path(FIG_DIR_084, 
  ...
  if (length(plots_list) > 0) {
    ...
    # 保存组合图
    combined_filename <- file.path(FIG_DIR_084, 
                                 paste0(gsub("PDM_Experience_", "", pdm_var), 
    ...
    # 也保存为PNG以便于查看
    png_filename <- file.path(FIG_DIR_084, 
                            paste0(gsub("PDM_Experience_", "", pdm_var), 
...
# 保存汇总表
summary_table_path <- file.path("results/tables", paste0("084_pdm_experience_pie_charts_k", K_VALUE_FIXED))
if (!dir.exists(summary_table_path)) {
  dir.create(summary_table_path, recursive = TRUE)
  cat(paste("Created directory:", summary_table_path, "\n"))
}

summary_filename <- file.path(summary_table_path, paste0("pdm_experience_distribution_k", K_VALUE_FIXED, ".csv"))
write_csv(experience_summary, summary_filename)
cat("Summary table saved to:", summary_filename, "\n")

cat("\n============== SCRIPT R/084 (PDM Experience Pie Charts k7) FINISHED ==============\n") 