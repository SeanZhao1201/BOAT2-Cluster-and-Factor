# R/093_project_success_pie_charts_k4.R
# This script visualizes the distribution of Project_Success for each of the k=4 clusters
# using pie charts. Data is loaded from the output of script 090.

# 0. Load Setup and Data -----------------------------------------------------
cat("============== SCRIPT R/093 (Project Success Pie Charts k4) STARTING ==============\n")
...
INPUT_TABLES_DIR_090 <- paste0("results/tables/090_kprototype_post_1_removal_k", K_VALUE_FIXED) # <--- CHANGED
...
# Create subdirectory for results from this script (093)
FIG_DIR_093 <- paste0("results/figures/093_project_success_pie_charts_k", K_VALUE_FIXED) # <--- CHANGED
if (!dir.exists(FIG_DIR_093)) {
  dir.create(FIG_DIR_093, recursive = TRUE)
  cat(paste("Created directory:", FIG_DIR_093, "\n"))
}
...
cat("Configuration complete. Output will be saved to:", FIG_DIR_093, "\n")
...
for (cluster_num in 1:K_VALUE_FIXED) {
  ...
  individual_plot_filename <- file.path(FIG_DIR_093, # <--- CHANGED
...
if (length(plots_list) > 0) {
  num_cols_grid <- if (K_VALUE_FIXED == 4) 2 else 3 # <--- CHANGED for k=4
  num_rows_grid <- ceiling(length(plots_list) / num_cols_grid)
  
  combined_plot_filename <- file.path(FIG_DIR_093, # <--- CHANGED
                                    paste0("all_clusters_pie_charts_k", K_VALUE_FIXED, ".pdf"))
  
  # Save combined plot with adjusted dimensions for k=4 (2x2 grid)
  pdf_width <- if (K_VALUE_FIXED == 4) num_cols_grid * 8 else num_cols_grid * 7 # Wider for fewer columns
  ...
  # Ensure legend settings are consistent for all plots in the grid
  ...
  # Using grid.arrange to combine plots and add a main title
  grid.arrange(
    grobs = plots_list, 
    ncol = num_cols_grid,
    nrow = num_rows_grid,
...
cat("\n============== SCRIPT R/093 (Project Success Pie Charts k4) FINISHED ==============\n") 